...
public class MBeanAttributeRatioLoadMetricTestCase {
    ...
    public void getLoad() throws Exception {
        ...
        when(mbeanServer.getAttribute(same(name1), same(dividend))).thenReturn(1);
        when(mbeanServer.getAttribute(same(name2), same(dividend))).thenReturn(2);

        when(mbeanServer.getAttribute(same(name1), same(divisor))).thenReturn(2);
        when(mbeanServer.getAttribute(same(name2), same(divisor))).thenReturn(2);
